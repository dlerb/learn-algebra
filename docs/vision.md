...
**Tier 1 — Equivalence fluency (Read):** *Static* recognition of when two expressions are — and are not — the same object. Each Tier-1 skill is a source-layer statement (a definition, axiom, or theorem) made student-facing: *rendered* through conventions, *equated* by a law. Knowing that `3x`, `3·x`, `3×x`, `x·3` are one object (convention); that `ab = ba` (axiom); that `a² = a·a` (definition); reading `-(a-b)`, minus signs, and brackets correctly however they appear. Just as central are the tempting **non-**equivalences to resist: `a²` is not `2a`, `a²+b²` is not `(a+b)²`. These equivalences are the atoms Tier 3 later chains toward a goal — Tier 1 is knowing them cold, not searching for which to apply.

**Tier 2 — Structural recognition (Classify):** The ability to identify the dominant operation of an expression before acting on it. Seeing `3(x+1) − 2(x−1)` as "fundamentally a subtraction between two products." Classifying the shape first, computing second.

**Tier 3 — Manipulation (Transform):** Applying equivalence rules to rewrite expressions. This introduces its own "three ways to do it" problem: multiple valid manipulation paths exist (expand, factor, rearrange), all correct but leading to different results. Choosing the right one requires having classified the structure first (Tier 2). Tier 3 cannot be fluent without Tiers 1 and 2 as foundation.
...
*Corollary — skill membership belongs to skills and drills, not to expressions.* Asking whether an expression like `3a(b+c)` "is Tier 1 or Tier 3" is malformed. The *skill* `a(b+c) = ab+ac` is Tier 1 (a static equivalence — a fact). `3a(b+c)` is a Tier-3 *drill* that deploys that skill (with `a := 3a`) and composes it with another Tier-1 skill (regrouping `3a·b → 3ab`). Tier-3 difficulty just *is* how many Tier-1 atoms a drill composes; Tier 1 is the degenerate floor (one atom, bare pattern, recognised not produced). So the Tier-1↔Tier-3 line is a **generation parameter, not a taxonomy decision**: when a skill's slots range over single *atoms* it is a Tier-1 recognition drill; when they are filled with *compound or nested* terms that force further normalisation, the same skill becomes a Tier-3 drill. Keeping this straight is the filter that stops the taxonomy from bloating with near-duplicate "harder" skills.
...
- Tier 1 and Tier 2 exercises require no computation — pure reading and classification
