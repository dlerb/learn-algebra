...
- [x] Skill schema as Zod (`src/data/skill.schema.ts`) — single source for validator + `Skill` type
  - [x] `kind` discriminator = the skill's *mental step*: `equivalence` (equal-forms set), `recognition` (equal-forms set, but a Skill-2 "same value across different structure" step), `classification` (`examples` + `answer`), `chunking` (`examples[].chunks`); `transformation` reserved for Skill 3.
...
- [ ] Generation params: per-skill, which letters vary over which pools (respect shared binding for equivalence, independent for `structure`). **The pool choice is the Skill-1↔Skill-3 selector** (atoms → S1 recognition drill; compound/nested → S3 drill of the same skill) — see vision.md #4 corollary.
...
*requires* both. A S3 skill = S1 equivalences + S2 parsing + **a target**.
...
- [ ] Author Skill 3 skills; wire the `gateway` shapes as their recognition triggers
- [ ] **Coefficient-lens chunking** (`2x(x+1) → [2x, (x+1)]`, keeping the coefficient with its variable): operation-relative grouping, the recognition-half of an *expand* move — author as a Skill-3 skill. The example was removed from the Skill-2 `chunking.chunks-in-product` card on 2026-07-11 (Skill-2 chunking is maximal/flat: `[2, x, (x+1)]`); this is where its coefficient-lens version lands.
