...
- [~] **Layer 1+2 — laws & notation devices** (`docs/laws_and_devices.md`, draft): axioms/definitions/theorems/anti-laws + device list as the coordinate system families live in (justify + audit, not generate). Next: confirm draft, then `laws.json` + `devices.json` + `justifiedBy`/`devices` fields on families + matrix-audit validator; tag all 64 families during the inspection pass.
